...
def event_times(outputs, dep, options):
    ...
    if options['mode'] == 'potency':
        print_string = 'Potency rate'
        target_var = np.trapz(sliprate, x=z*1e3, axis=0)
        ipsr = np.argmax(sliprate, axis=0)
        if options['SSE']:
            detec_thresh = np.min(target_var) * (1 + options['Pths_mult'])
            thresh_seismic = np.max(target_var) * options['Pths_mult_seismic']
        else:
            detec_thresh = np.max(target_var) * options['Pths_mult']
    elif options['mode'] == 'sliprate':
    ...
    if options['SSE']:
        ...
        if options['print_on']: print('Event criteria: %1.0e < %s < %1.0e'%(detec_thresh, print_string, thresh_seismic))
        ...
        if options['print_on']: print('Event criteria: %s > %1.0e'%(print_string, detec_thresh))
        base = np.where(target_var < detec_thresh)[0]

    if options['mode'] == 'potency':
        # If the peaks are based on the potency rate, make sure their slip rate is also seismic level
        peaks = peaks[np.max(sliprate, axis=0)[peaks] > options['Vths']]
